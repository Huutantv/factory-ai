...
impl MarkdownStream {
    ...
    fn emit_code(&self, out: &mut String, line: &str) {
        let left = format!("{}{}", gutter(), theme::code_rule("│ "));
        let inner = self.fence_inner();
        let right = format!(" {}", theme::code_rule("│"));
        ...
        if c == '`' {
            if let Some(close) = find_char(&chars, i + 1, '`') {
                if close > i + 1 {
                    ...
                    out.push_str(&theme::link(code).to_string());
                    i = close + 1;
                    continue;
                }
            }
...
fn highlight(line: &str, lang: &str) -> String {
    ...
    while i < chars.len() {
        ...
        if comments.iter().any(|t| starts_with_at(&chars, i, t)) {
            ...
            out.push_str(&theme::code_comment(rest).to_string());
            break;
        }
        // string literal (" or ')
        if c == '"' || c == '\'' {
            ...
            out.push_str(&theme::code_string(s).to_string());
            i = j;
            continue;
        }
        // number
        if c.is_ascii_digit() && (i == 0 || !is_ident_char(chars[i - 1])) {
            ...
            out.push_str(&theme::code_number(s).to_string());
            i = j;
            continue;
        }
        // identifier / keyword
        if is_ident_start(c) {
            ...
            if is_keyword(&word) {
                out.push_str(&theme::code_keyword(&word).to_string());
            } else {
                out.push_str(&word);
            }
            i = j;
