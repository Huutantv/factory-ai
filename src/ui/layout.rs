...
//! `default`, `polished`, `dense`, and `spacious`; custom layouts live in
...
static CURRENT: once_cell::sync::Lazy<std::sync::RwLock<LayoutConfig>> =
    once_cell::sync::Lazy::new(|| std::sync::RwLock::new(LayoutConfig::default()));
...
/// Activate a layout without persisting it; used by UI presets that save once after both parts apply.
pub fn set_layout_live(name: &str) -> Option<LayoutConfig> {
...
mod tests {
    ...
    fn responsive_breakpoints_are_stable() {
        ...
        assert_eq!(layout_for_columns(80).name, "polished");
        assert_eq!(layout_for_columns(119).name, "polished");
