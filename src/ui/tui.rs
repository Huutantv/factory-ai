    ...
    loop {
        ...
        let key = loop {
            ...
            if let Event::Paste(text) = ev {
                let text = normalize_paste_text(&text);
                // Some Windows terminals deliver Ctrl+V as an empty bracketed-paste event when
                // the clipboard contains a bitmap rather than text. In that case read the image
                // clipboard explicitly; otherwise the event is consumed with no visible feedback.
                if text.is_empty() {
        ...
        match key {
            ...
            Key::Enter => {
                ...
                // A dragged/dropped or typed image path arrives as ordinary terminal text. Resolve
                // those paths at submit time, remove them from the prompt, and merge them with any
                // clipboard images collected by Ctrl-O. This also makes quoted paths with spaces work.
                let (line, file_images) = crate::ui::image_input::extract_image_attachments(&line);
                let images = images + file_images.len();
                // A path can begin with `/` on Unix, so `images > 0` below prevents it from being
                // mistaken for a slash command after it has been recognized as an attachment.
                ...
                match crate::features::slash::classify(&trimmed).filter_command(images == 0) {
                    ...
                    crate::features::slash::Verdict::Chat => {
                        ...
                        let mut imgs = take_pending_images();
                        imgs.extend(file_images);
            ...
            Key::Char('\u{16}') => {
                // Ctrl-V prefers an image on the clipboard, which makes screenshot paste behave
                // like users expect in a vision chat. If the clipboard has no image, retain the
                // existing text-paste behavior for ordinary prose and paths. Ctrl-O remains a
                // compatibility shortcut for the same image action.
                if let Ok(Some(url)) = crate::ui::image_input::clipboard_image_data_url() {
                    push_pending_image(url);
                    render().lock().unwrap().images = pending_image_count();
                    hist_idx = None;
                    ...
                    pending_paste_repaint = false;
                    repaint();
                } else if paste_clipboard_into_draft(&mut paste_echo).is_some() {
                    hist_idx = None;
                    last_arrival = None;
                    last_arrival_prev = None;
                    pending_paste_repaint = false;
