...
/// 5-row block glyphs (each exactly 5 columns) for the letters in the Factory Auto wordmark.
/// [`push_title`] is only called with supported uppercase letters; unsupported characters render blank.
...
/// Append the Factory Auto logo: the sun mark, then the block-art wordmark (silver gradient), then the
/// tagline.
fn push_title(out: &mut String, word: &str, allow_sixel: bool) {
    ...
    let _ = writeln!(
        out,
        "  {}",
        style("FACTORY AUTO").color256(crate::ui::theme::MUTED)
    );
    out.push('\n');
}
...
fn render_inner(allow_sixel: bool) -> String {
    ...
    push_title(&mut out, "FACTORY AUTO", allow_sixel);
    ...
    boxline(
        ...
        &format!(
            "{} {}",
            style("F.Auto").color256(ACCENT).bold(),
            style(format!(
                "v{} · {} · {}",
...
pub fn welcome() -> String {
    let mut out = String::new();
    push_title(&mut out, "FACTORY AUTO", true);
    let _ = writeln!(
        out,
        "  {}",
