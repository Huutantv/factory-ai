...
struct CacheKey {
    ...
    hash: u64,
    complete: bool,
    /// Active theme tag — cached rows bake SGR colours at render time, so a `/theme`
    /// switch must miss the cache or old colours survive (the "không thay đổi" bug).
    theme_tag: u64,
    /// Dense `compact_tools` changes the tool-row shape, so it is part of the key too.
    compact: bool,
}
...
impl RenderCache {
    fn get_or_render(&mut self, block: &UiBlock, width: u16) -> Vec<String> {
        // Theme + compact flag participate so a live `/theme`/`/layout` switch re-renders
        ...
        use std::hash::{Hash, Hasher};
        pal.accent.hash(&mut th);
        pal.accent_dim.hash(&mut th);
        pal.muted.hash(&mut th);
        pal.faint.hash(&mut th);
        pal.ok.hash(&mut th);
        pal.err.hash(&mut th);
        pal.warn.hash(&mut th);
        pal.link.hash(&mut th);
        pal.code_keyword.hash(&mut th);
        pal.code_string.hash(&mut th);
        pal.code_number.hash(&mut th);
        pal.code_comment.hash(&mut th);
        pal.code_rule.hash(&mut th);
        let key = CacheKey {
            id: block.id,
            width,
            ...
            theme_tag: th.finish(),
            compact: crate::ui::layout::current().compact_tools,
        };
...
fn apply_command(state: &mut AppState, cmd: Command) {
    match cmd {
        ...
        Command::ScrollTo(start) => {
            ...
            let visible = state.input.overlay.as_ref().map(|_| 0).unwrap_or(
                ROWS.load(Ordering::Relaxed)
                    .saturating_sub(footer_chrome_rows(&crate::ui::layout::current()))
