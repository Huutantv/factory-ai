...
#[test]
fn footer_chrome_rows_follow_header_and_footer_visibility() {
    let mut config = LayoutConfig::default();
    assert_eq!(footer_chrome_rows(&config), 3);

    config.show_header = false;
    assert_eq!(footer_chrome_rows(&config), 2);

    config.show_footer = false;
    assert_eq!(footer_chrome_rows(&config), 0);
}

#[test]
fn responsive_layouts_keep_the_transcript_floor_at_common_terminal_sizes() {
    for (width, expected) in [
        ...
        let layout = crate::ui::layout::layout_for_columns(width);
        assert_eq!(layout.name, expected);
        assert!(layout.message_separation <= 2);
        assert!(layout.message_padding_left <= 2);
        assert!(layout.max_input_rows >= 5);
    }
}
...
fn an_empty_draft_is_one_placeholder_row() {
    ...
    assert_eq!(layout.hint, "↵ send · Tab complete · Ctrl-O attach");
}
...
#[test]
fn tool_row_clips_long_target_to_the_frame_width() {
    let ev = ToolEvent {
        seq: 1,
        icon: "⚙".into(),
        name: "file_read".into(),
        target: "src/a/very/long/path/that/must/not/overflow.rs".into(),
        digest: String::new(),
        state: ToolState::Running,
        elapsed_ms: None,
    };
    let row = plain(&render_tool_row(&ev, 30));
    assert!(
    ...
    assert!(
        row.ends_with('…'),
        "long target should be visibly clipped: {row:?}"
    );
}
