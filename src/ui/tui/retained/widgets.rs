...
pub(crate) fn render_tool_row(t: &ToolEvent, width: usize) -> String {
    ...
    let call_line = if t.target.is_empty() {
        theme::accent(&call_prefix).to_string()
    } else {
    ...
    // Dense layout: single compact line `icon name — digest · time` (opencode `dense` style).
    if crate::ui::layout::current().compact_tools {
        let digest_budget = width.saturating_sub(console::measure_text_width(&call_prefix) + 5);
        let digest = clip_to(&t.digest, digest_budget);
        ...
        return if icon.trim().is_empty() {
            ...
            format!("{}{}", theme::accent(&icon), compact)
        };
    }
