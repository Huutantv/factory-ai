...
pub(super) fn draw(frame: &mut Frame<'_>, state: &mut AppState) {
    ...
    let layout_cfg = crate::ui::layout::current();
    ...
    let footer_rows = footer_chrome_rows(&layout_cfg).saturating_add(layout.rows.len() as u16);
    let footer_rows = footer_rows.min(area.height.saturating_sub(1).max(1));
...
pub(super) fn footer_chrome_rows(config: &crate::ui::layout::LayoutConfig) -> u16 {
    u16::from(config.show_header) + if config.show_footer { 2 } else { 0 }
}

pub(super) fn max_input_rows(height: u16) -> usize {
    const MIN_TRANSCRIPT_ROWS: u16 = 3;
    let chrome_rows = footer_chrome_rows(&crate::ui::layout::current());
    let spare = height
        .saturating_sub(chrome_rows)
        .saturating_sub(MIN_TRANSCRIPT_ROWS);
    // The active layout caps the composer height (`dense` = 5); the hard ceiling stays.
    let cap = (crate::ui::layout::current().max_input_rows as u16).clamp(1, MAX_INPUT_TEXT_ROWS);
    spare.clamp(1, cap) as usize
}
...
pub(super) fn draw_transcript(frame: &mut Frame<'_>, area: Rect, state: &mut AppState) {
    ...
    let padding = crate::ui::layout::current().message_padding_left as u16;
    let transcript = Rect {
        x: area.x.saturating_add(padding).min(area.right()),
        y: area.y,
    ...
    let content_width = transcript.width.saturating_sub(2).max(8);
    let mut lines: Vec<Line<'static>> = Vec::new();
    let mut plain_rows: Vec<String> = Vec::new();
    let mut sgr_rows: Vec<String> = Vec::new();
    // opencode-style layout: blank-line separation between blocks comes from the active
    // layout (`default` = 1 like before, `dense` = 0). Read once per frame — never per block.
    let layout = crate::ui::layout::current();
    let mut first_block = true;
    for block in &state.blocks {
        if !first_block {
            for _ in 0..layout.message_separation.min(3) {
                plain_rows.push(String::new());
                sgr_rows.push(String::new());
                lines.push(Line::raw(""));
            }
        }
        first_block = false;
    ...
    frame.render_widget(paragraph, transcript);
    ...
    if total > visible {
        ...
        let scrollbar = Scrollbar::new(ScrollbarOrientation::VerticalRight)
            ...
            .track_symbol(Some("│"))
            .thumb_symbol("█")
            .style(Style::default().fg(Color::Indexed(crate::ui::theme::muted_idx())))
            .track_style(Style::default().fg(Color::Indexed(crate::ui::theme::faint_idx())));
        frame.render_stateful_widget(scrollbar, transcript, &mut sb_state);
    }
    ...
    let jump_button = if total > visible && start < tail_start {
        ...
        if transcript.width > label_w + 1 && transcript.height >= 1 {
            let bx = transcript.x + transcript.width - 1 - label_w;
            let by = transcript.y + transcript.height - 1;
...
pub(super) fn working_line(state: &AppState) -> Vec<(String, Line<'static>)> {
    ...
    let spans = vec![
        Span::styled(
            format!("{glyph} "),
            Style::default().fg(Color::Indexed(pal.accent)),
        ),
        Span::styled(revealed, Style::default().fg(Color::Indexed(pal.link))),
        Span::styled(
...
pub(super) fn draw_footer(
    ...
    // `draw` asked for the active chrome rows plus the composer rows, but a Layout under pressure
    // can hand back less than it asked for — so paint what actually arrived rather than what was requested.
    let layout_cfg = crate::ui::layout::current();
    let chrome_rows = footer_chrome_rows(&layout_cfg);
    let text_rows = layout
        .rows
        .len()
        .min(area.height.saturating_sub(chrome_rows) as usize)
        .max(1);
    let rows = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(u16::from(layout_cfg.show_header)),
            Constraint::Length(u16::from(layout_cfg.show_footer)),
    ...
    // Active theme drives the input-box accent (live, cached — no disk in draw path).
    let pal = crate::ui::theme::current_palette();
    ...
    let right_spans: Vec<Span<'static>> = {
        ...
        if !layout_cfg.show_context_meter {
            // Dense layout: health chip only, no context bar (opencode `dense` style).
            vec![Span::styled(
                format!("● {}", state.health.label(false)),
                health_style,
            )]
        } else {
            ...
            let faint = Style::default().fg(Color::Indexed(pal.faint));
            let muted = Style::default().fg(Color::Indexed(pal.muted));
            vec![
...
fn placeholder_text(state: &AppState) -> String {
    ...
    if state.working {
        format!("Queue · Alt+↵ steer · Esc stop{q}{s}")
    } else if state.input.images > 0 {
        format!(
            "{} attachment{} ready · / commands{q}",
            state.input.images,
            if state.input.images == 1 { "" } else { "s" }
        )
    } else {
...
pub(super) fn input_layout(state: &AppState, width: usize, max_rows: usize) -> InputLayout {
    ...
    if state.input.draft.is_empty() {
        let hint = if crate::ui::layout::current().show_input_hints && state.input.overlay.is_none()
        {
            "↵ send · Tab complete · Ctrl-O attach"
        } else {
            ""
        };
