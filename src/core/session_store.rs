...
pub(crate) struct SessionMeta {
    ...
    /// Persisted todo list — the plan survives process restart.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub(crate) fn save_session(history: &[Message], name: &str, model: Option<&str>) -> Result<String> {
    ...
    write_session(
        ...
        SessionMeta {
            ...
            created: None,
            updated: None,
            todos: crate::agent::todo::serialize_for_session(),
        },
    )
}
...
pub(crate) fn load_session(history: &mut Vec<Message>, name: &str, model: &str) -> Result<usize> {
    ...
    *history = loaded;
    ...
    // Restore the persisted todo list if present
    if let Some(ref m) = meta {
        if let Some(ref todos_value) = m.todos {
            if let Err(e) = crate::agent::todo::restore_from_session(todos_value) {
                // Non-fatal: log but don't fail the load
                eprintln!("Warning: could not restore todos: {e:#}");
            }
        }
    }
