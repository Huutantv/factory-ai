...
pub struct CliConfig {
    ...
    /// TUI theme name (opencode-style): a built-in (`moonlight`, `tokyonight`, `catppuccin`,
    /// `gruvbox`, `nord`, `matrix`, `one-dark`) or a custom theme under `~/.aizen/themes/*.json`.
    /// `None` ⇒ `moonlight` (current default). Set live with `/theme`, persisted here.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub theme: Option<String>,
    /// TUI layout name (opencode-style): built-in `default` / `polished` / `dense` / `spacious`,
    /// `auto`, or a custom layout under `~/.aizen/layout/*.{json,jsonc}`. `None` ⇒ `default`.
    /// Set live with `/layout`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
