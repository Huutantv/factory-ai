...
pub(crate) struct AgentArgs {
    ...
    /// Plan-only mode: generate a step-by-step plan without executing any tools. The model uses
    /// read-only tools (search, grep, read) to understand the task and produces a detailed plan,
    /// but does not edit files or run shell commands. Useful for reviewing what would be done
