...
//! Claude Code's todo list is one of its most legible features. Persisted to session files so
//! plans survive process restart, rendered as a checklist into the scroll region + summarised
//! in the status bar. Reset on `/clear`.
//!
...
/// Serialize the current todo list for session persistence.
pub fn serialize_for_session() -> Option<serde_json::Value> {
    let items = snapshot();
    if items.is_empty() {
        None
    } else {
        Some(serde_json::to_value(&items).unwrap_or(serde_json::Value::Null))
    }
}

/// Restore a todo list from a persisted session.
pub fn restore_from_session(value: &serde_json::Value) -> Result<()> {
    let items: Vec<Todo> = serde_json::from_value(value.clone())
        .map_err(|e| anyhow::anyhow!("invalid persisted todos: {e}"))?;
    set(normalize_todos(items));
    Ok(())
}
...
/// Generate a completion summary: planned N · done M · remaining K.
/// Returns None if there are no todos.
pub fn completion_summary() -> Option<String> {
    let items = snapshot();
    if items.is_empty() {
        return None;
    }
    ...
    if remaining == 0 {
        Some(format!("✓ All {total} planned task(s) completed"))
    } else {
        Some(format!(
            "Plan: {total} task(s) · {done} done · {remaining} remaining"
        ))
    }
}

/// Count of tasks by status for detailed reporting.
pub fn todo_stats() -> (usize, usize, usize, usize) {
    let items = snapshot();
    let total = items.len();
    let done = items.iter().filter(|t| t.status == Status::Done).count();
    let in_progress = items
        ...
        .count();
    let pending = items.iter().filter(|t| t.status == Status::Pending).count();
    (total, done, in_progress, pending)
}

/// Count of done todos (for the evidence ledger).
pub fn todo_done_count() -> usize {
    snapshot()
        .iter()
...
mod tests {
    ...
    #[test]
    fn serialize_and_restore_todos() {
        let _g = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());

        // Set up a todo list
        set(vec![
            Todo::new("first task", Status::Done),
            Todo::new("second task", Status::InProgress),
            Todo::new("third task", Status::Pending),
        ]);

        // Serialize
        let serialized = serialize_for_session().expect("should serialize");

        // Clear and restore
        clear();
        assert_eq!(snapshot().len(), 0);

        restore_from_session(&serialized).expect("should restore");
        let restored = snapshot();
        assert_eq!(restored.len(), 3);
        assert_eq!(restored[0].content, "first task");
        assert_eq!(restored[0].status, Status::Done);
        assert_eq!(restored[1].content, "second task");
        assert_eq!(restored[1].status, Status::InProgress);
        assert_eq!(restored[2].content, "third task");
        assert_eq!(restored[2].status, Status::Pending);

        clear();
    }

    #[test]
    fn empty_todo_list_serializes_to_none() {
        let _g = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        clear();
        assert!(serialize_for_session().is_none());
    }

    #[test]
    fn completion_summary_with_all_done() {
        let _g = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        set(vec![
            Todo::new("task 1", Status::Done),
            Todo::new("task 2", Status::Done),
        ]);
        let summary = completion_summary().expect("should have summary");
        assert!(
            summary.contains("All 2 planned task(s) completed"),
            "{summary}"
        );
        clear();
    }
    ...
    #[test]
    fn completion_summary_with_incomplete() {
        let _g = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        set(vec![
            Todo::new("task 1", Status::Done),
            Todo::new("task 2", Status::InProgress),
            Todo::new("task 3", Status::Pending),
        ]);
        let summary = completion_summary().expect("should have summary");
        assert!(summary.contains("3 task(s)"), "{summary}");
        assert!(summary.contains("1 done"), "{summary}");
        assert!(summary.contains("2 remaining"), "{summary}");
        clear();
    }

    #[test]
    fn completion_summary_none_when_empty() {
        let _g = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        clear();
    ...
    #[test]
    fn todo_stats_counts_correctly() {
        let _g = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        set(vec![
            Todo::new("done 1", Status::Done),
            Todo::new("done 2", Status::Done),
            Todo::new("active", Status::InProgress),
            Todo::new("pending 1", Status::Pending),
            Todo::new("pending 2", Status::Pending),
        ]);
        let (total, done, in_progress, pending) = todo_stats();
        assert_eq!(total, 5);
        assert_eq!(done, 2);
