...
/// Run plan-only mode: spawn a read-only planner sub-agent to produce a step-by-step plan
/// without executing any destructive operations. The planner can read files, search, and use
/// memory, but cannot edit or run shell commands.
async fn run_plan_only_mode(
    http: &reqwest::Client,
    base_url: &str,
    api_key: &str,
    model: &str,
    ...
    // Build a planner-scoped (read-only) registry. The planner role gets: read/glob/search,
    // memory, web research, skill_load, LSP navigation, but no file_edit, no shell_run, no task.
    let registry = agent::builtin::role_registry("planner", &std::path::PathBuf::from(&cwd));

    // Build the planner sub-agent prompt with the contract to produce a concrete ordered plan.
    let system = agent::task_tool::build_subagent_prompt(
        "planner",
        &[],
        &std::path::PathBuf::from(&cwd),
        model,
        &date,
        None, // no contract budget
        Some(frozen),
    );

    // Non-interactive config: quiet mode, no verify gate (read-only), tight budget.
    let cfg = AgentConfig {
        max_iters: 15,
        auto_extend_to: 15,
        approval_mode: ApprovalMode::Yolo, // read-only tools don't need approval
        quiet: true,
        enable_verify_gate: false,
        context_window: resolve_ctx_window(model).0,
        enable_lsp: crate::agent::lsp::LSP.is_enabled(),
        deadline: None, // no wall-clock deadline for a top-level plan-only run
        ..Default::default()
    };

    // The model call, same as run_agent_cmd but non-streaming (sub-agent convention).
    let http_ref = http;
    let base = base_url;
    let key = api_key;
    let model_ref = model;
    ...
    // Run the planner loop: it will call todo_write (ScopedTodo for sub-agents) to build a plan.
    let mut history = vec![Message::system(&system), Message::user(task.trim())];
    ...
    if let Some(final_text) = &outcome.final_text {
        ...
        println!("(the planner produced no output)\n");
    }

    // If the planner stopped uncleanly, surface that.
    match outcome.stop {
        StopReason::Done => {
            eprintln!(
                "{}",
                style(
                    "✓ Plan generation complete. Use `aizen agent` without --plan-only to execute."
                )
                .dim()
            );
        }
...
pub(crate) async fn run_agent_cmd(args: AgentArgs) -> Result<()> {
    ...
    // Plan-only mode: spawn a read-only planner sub-agent, print the plan, and exit without executing.
    if args.plan_only {
        return run_plan_only_mode(&http, &base_url, &api_key, &model, &args.task, &frozen).await;
    }
