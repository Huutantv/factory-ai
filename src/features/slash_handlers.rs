...
/// Arrow-key theme picker for bare `/theme` (opencode `/theme` dialog equivalent).
/// Runs SUSPENDED with stdin to itself. Esc keeps the current theme.
fn theme_picker(history: &[types::Message], model_label: &str) {
    use dialoguer::Select;
    let names = crate::ui::theme::list_themes();
    if names.is_empty() {
        tui::emit_line(&style("no themes available").dim().to_string());
        return;
    }
    let current = crate::ui::theme::current_palette().name;
    ...
    let default = names
        ...
        .unwrap_or(0);
    match Select::with_theme(&crate::ui_theme())
        .with_prompt("Theme — ↑/↓ to pick, Enter to apply, Esc to keep current")
        .items(&items)
        .default(default)
        .interact_opt()
    {
        Ok(Some(i)) => {
            if let Some(pal) = crate::ui::theme::set_theme(&names[i]) {
                tui::set_status(&crate::status_text(history, model_label));
                // `println!` while suspended (real terminal); the retained transcript also gets
                // the confirmation line on resume via `emit_line`.
                println!(
                    "{}",
                    style(format!("theme → {} — applied live", pal.name))
                        .color256(crate::ui::theme::accent_idx())
                );
                tui::emit_line(
                    &style(format!("theme → {} — applied live", pal.name))
                        .color256(crate::ui::theme::accent_idx())
        ...
        _ => {
            println!("{}", style("(kept current theme)").dim());
        }
    }
}

/// Arrow-key layout picker for bare `/layout` (opencode `/layout` dialog equivalent).
fn layout_picker() {
    use dialoguer::Select;
    ...
    let default = names
        ...
        .unwrap_or(0);
    match Select::with_theme(&crate::ui_theme())
        .with_prompt("Layout — ↑/↓ to pick, Enter to apply, Esc to keep current")
        .items(&items)
        .default(default)
        .interact_opt()
    {
    ...
    match builtin.id {
        ...
        SlashId::Preset => {
            ...
            if sub.is_empty() || sub.eq_ignore_ascii_case("list") {
                for line in [
                    "presets:",
                    "  polished — moonlight theme + polished layout",
                    "  midnight — tokyonight theme + spacious layout",
                    "  minimal — nord theme + dense layout",
                ] {
                    tui::emit_line(&style(line).dim().to_string());
                }
            } else {
                let (theme, layout) = match sub.to_ascii_lowercase().as_str() {
                    "polished" => ("moonlight", "polished"),
                    "midnight" => ("tokyonight", "spacious"),
                    "minimal" => ("nord", "dense"),
                    _ => {
                        tui::emit_line(&style(format!("unknown preset {sub:?} — /preset list")).dim().to_string());
                        return SlashOutcome::Continue;
                    }
                };
                let Some(pal) = crate::ui::theme::set_theme_live(theme) else {
                    tui::emit_line(&style(format!("preset theme {theme:?} is unavailable")).dim().to_string());
                    return SlashOutcome::Continue;
                };
                let Some(layout_cfg) = crate::ui::layout::set_layout_live(layout) else {
                    tui::emit_line(&style(format!("preset layout {layout:?} is unavailable")).dim().to_string());
                    return SlashOutcome::Continue;
                };
                let mut cfg = crate::core::cli_config::load();
                cfg.theme = Some(theme.to_string());
                cfg.layout = Some(layout.to_string());
                let _ = crate::core::cli_config::save(&cfg);
                tui::set_status(&crate::status_text(history, model_label));
                tui::emit_line(&style(format!("preset → {} — theme={} layout={}", sub, pal.name, layout_cfg.name)).color256(crate::ui::theme::accent_idx()).to_string());
            }
        }
        SlashId::Theme => {
            let sub = arg.trim();
            if sub.is_empty() {
                // Bare `/theme` runs SUSPENDED (Stdin::WhenBare) with stdin to itself, so an
                // arrow-key dialoguer picker works like opencode's `/theme` dialog.
                theme_picker(history, model_label);
            } else if sub.eq_ignore_ascii_case("list") {
                let current = crate::ui::theme::current_palette().name;
                let mut lines = vec![format!("theme: {current} (active)")];
                for name in crate::ui::theme::list_themes() {
                    if !name.eq_ignore_ascii_case(&current) {
                        lines.push(format!("  {name}"));
                    }
                }
                lines.push("tip: bare /theme opens the arrow-key picker".to_string());
                for l in lines {
                    tui::emit_line(&style(l).dim().to_string());
                }
            } else if let Some(pal) = crate::ui::theme::set_theme(sub) {
                tui::set_status(&crate::status_text(history, model_label));
                tui::emit_line(
                    &style(format!("theme → {} — accent/ok/err/warn/link recoloured live", pal.name))
                        .color256(crate::ui::theme::accent_idx())
                        .to_string(),
                );
            } else {
                tui::emit_line(
                    &style(format!("unknown theme {sub:?} — /theme list to see options"))
                        .dim()
                        .to_string(),
                );
            }
        ...
        SlashId::Layout => {
            ...
            if sub.is_empty() {
                ...
                let current = crate::ui::layout::current().name;
                let mut lines = vec![format!("layout: {current} (active)")];
                for name in crate::ui::layout::list_layouts() {
                    if !name.eq_ignore_ascii_case(&current) {
                        lines.push(format!("  {name}"));
                    }
                }
                ...
                tui::emit_line(
                    &style(format!(
                        "layout → {} — separation={} padding={} compact_tools={} ctx_meter={}",
                        l.name,
                        l.message_separation,
                        l.message_padding_left,
                        l.compact_tools,
                        l.show_context_meter
                    ))
                    .dim()
                    .to_string(),
                );
                ...
                tui::emit_line(
                    &style(format!("unknown layout {sub:?} — /layout list to see options"))
                        .dim()
                        .to_string(),
                );
            }
